...
ggplot(data = state_df,
            mapping = aes(x = long, y = lat,
                          group = group)) +
  geom_polygon(fill = "white", color = "black") + coord_map(projection = "albers", lat0 = 39, lat1 = 45) + theme_void() # eliminates all the stuff except the polygons
...
# Exercise 1: 
state_pop = state_full %>% mutate(popchange = 100 * (pop2010 - pop2000)/pop2000)
ggplot(data = state_pop, aes(x = long, y = lat, group = group)) +
  geom_polygon(colour = "black", aes(fill = popchange)) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  theme_void() +
  scale_fill_viridis_b()

# Exercise 2: diverging if below 0, so it is getting darker if growing
ggplot(data = state_pop, aes(x = long, y = lat, group = group)) +
  geom_polygon(colour = "black", aes(fill = popchange)) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  theme_void() +
  scale_fill_fermenter(type = "div", direction = -1)
# should have the color change be at 0 but we don't really know how to do that
# fermenter is for binning continuous
# brewer is for discrete colors
# distiller is for continuous variables 
