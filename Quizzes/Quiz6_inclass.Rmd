...
If you were trying to compare something like the female ratio across all of the majors, if would benefit you to have a static graph so that every major is visible at one time and the user can just look at the entire thing and not have to change anything. 
...
ui <- fluidPage(
  sidebarLayout(sidebarPanel(
    ...
    selectizeInput("playerchoice2",
                   label = "Choose a Different Player", choices = levels(factor(df$player)),
                   selected = "Petra Martic"),
...
ggplot(data = df_sub2(), aes(x = player, y = .data[[input$varchoice2]])) + geom_boxplot(bins = 15, fill = "white", color = "black")
  })
